...
public class FindResultsWindow : EditorWindow
{
	...
	private void OnGUI()
	{
		...
		if (replaceText != null)
		{
			...
			GUI.enabled = assetGuids.Count == 0;
			if (GUILayout.Toggle(false, foundNoResults || assetGuids.Count > 0 ? "Close" : "Replace all selected", boldToolbarButton, toolbarButtonLayoutOptions))
			{
				ReplaceAll();
			}
			GUI.enabled = true;
